...
template<const size_t N>
std::bitset<N-1> from_string(const char(&bits)[N]) {
...
void test_line_encoder()
{
    ...
    std::array test_sets_selflearning {
        std::make_pair(from_string("1100100100010110011111110100110"),
...
void test_fan_speed()
{
    ...
    TEST_ASSERT_EQUAL(7, PayloadEncoderBase::buildFanSpeed(0).to_ulong());
    TEST_ASSERT_EQUAL(6, PayloadEncoderBase::buildFanSpeed(1).to_ulong());
    TEST_ASSERT_EQUAL(0, PayloadEncoderBase::buildFanSpeed(7).to_ulong());
}
...
void test_brightness()
{
    ...
    TEST_ASSERT_EQUAL(63, PayloadEncoderBase::buildBrightness(0.0f).to_ulong());

    TEST_ASSERT_EQUAL(24, PayloadEncoderBase::buildBrightness(0.1f).to_ulong());
    TEST_ASSERT_EQUAL(41, PayloadEncoderBase::buildBrightness(0.5f).to_ulong());
    TEST_ASSERT_EQUAL(62, PayloadEncoderBase::buildBrightness(1.0f).to_ulong());
}
